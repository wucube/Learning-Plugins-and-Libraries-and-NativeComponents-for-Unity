var OriginalAudioContext = window.AudioContext || window.webkitAudioContext;window.AudioContext = function() {if (this instanceof window.AudioContext) {return wx.createWebAudioContext();} else {return new OriginalAudioContext();}};
        function getAttachmentName(attachment) {
            if (attachment == 0x8CE0) return "COLOR_ATTACHMENT0";
            else if (attachment == 0x8D00) return "DEPTH_ATTACHMENT";
            else if (attachment == 0x8D20) return "STENCIL_ATTACHMENT";
            else if (attachment == 0x821A) return "DEPTH_STENCIL_ATTACHMENT";
            else if (attachment == 0x8CE1) return "COLOR_ATTACHMENT1";
            else return "unknown";
        }
        function getInternalFormatName(internalFormat) {
            if (internalFormat == 0x1907) return "GL_RGB";
            else if (internalFormat == 0x1908) return "GL_RGBA";
            else if (internalFormat == 0x1909) return "GL_LUMINANCE";
            else if (internalFormat == 0x190A) return "GL_LUMINANCE_ALPHA";
            else if (internalFormat == 0x1906) return "GL_ALPHA";
            else if (internalFormat == 0x8229) return "GL_R8";
            else if (internalFormat == 0x8F94) return "GL_R8_SNORM";
            else if (internalFormat == 0x822D) return "GL_R16F";
            else if (internalFormat == 0x822E) return "GL_R32F";
            else if (internalFormat == 0x8232) return "GL_R8UI";
            else if (internalFormat == 0x8231) return "GL_R8I";
            else if (internalFormat == 0x881A) return "GL_RGBA16F";
            else if (internalFormat == 0x8814) return "GL_RGBA32F";
            else if (internalFormat == 0x8056) return "GL_RGBA4";
            else if (internalFormat == 0x8D62) return "GL_RGB565";
            else if (internalFormat == 0x8057) return "GL_RGB5_A1";
            else if (internalFormat == 0x81A5) return "GL_DEPTH_COMPONENT16";
            else if (internalFormat == 0x81A6) return "GL_DEPTH_COMPONENT24";
            else if (internalFormat == 0x8CAC) return "GL_DEPTH_COMPONENT32F";
            else if (internalFormat == 0x88F0) return "GL_DEPTH24_STENCIL8";
            else if (internalFormat == 0x8CAD) return "GL_DEPTH32F_STENCIL8";
            else if (internalFormat == 0x84f9) return "GL_DEPTH_STENCIL";
            else return "unknown:" + internalFormat;
        }
        GameGlobal.unityNamespace.ProfileWebgl = {
            DrawCallLimit : 250,
            ProgramSwitchLimit : 100,
            FboSwitchLimit : 5,
            RTSizeLimit : 1240,
            beginFrame: function () {
                var profileWebgl = GameGlobal.unityNamespace.ProfileWebgl;
                profileWebgl.curFrameNumber = Browser.mainLoop.currentFrameNumber | 0;
                profileWebgl.curFboSwitch = 0;
                profileWebgl.curProgramSwitch = 0;
                profileWebgl.curFboDrawCall = 0;
            },
            endFrame: function() {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var bShowLog = false;
                if (GameGlobal.unityNamespace.enableRenderAnalysisLog) {
                    bShowLog = (webglStats.curFrameNumber - webglStats.lastShowLogFrame > webglStats.showLogInterval) || (webglStats.lastShowLogFrame == 0);
                }
                var warningLog;
                var totalDc = 0;
                var logStr;
                if (bShowLog) {
                    logStr = ("Render fboSwitch:" + webglStats.curFboSwitch + ",programSwitch:" + webglStats.curProgramSwitch + "\n");
                    warningLog = "";
                    webglStats.lastShowLogFrame = webglStats.curFrameNumber;
                    if (webglStats.curFboSwitch > webglStats.FboSwitchLimit) {
                        warningLog += "Pass切换次数:" + webglStats.curFboSwitch + " 超过 " + webglStats.FboSwitchLimit + ",请减少或合并 RenderPass.\n";
                    }
                    if (webglStats.curProgramSwitch > webglStats.ProgramSwitchLimit) {
                        warningLog += "Program 切换次数:" + webglStats.curProgramSwitch + " 超过 " + webglStats.ProgramSwitchLimit + ",请优化材质，减少材质变体.\n";
                    }
                }
                for (var i = 0; i < webglStats.curFboSwitch; ++i) {
                    var fbo = webglStats.fboList[i];
                    if (bShowLog) logStr += ("index: " + i + ", fbo: " + webglStats.fboList[i] + "\n");  
                    var fboObj = webglStats.fboMap[fbo];
                    if (bShowLog && fboObj.drawcall == 0) {
                        warningLog += "存在没有 DrawCall 的Pass, fbo: " + fbo + ", 请优化掉.\n";
                    }
                    totalDc += fboObj.drawcall;
                    if (fbo == 0) {
                        if (bShowLog)logStr += ("--backbuffer: [" + UnityLoader.SystemInfo.width + "," + UnityLoader.SystemInfo.height + "], dc:" + fboObj.drawcall + ", program: " + fboObj.programSwitch + ",clear:" + fboObj.clearCount + ",invalidClear:"+fboObj.invalidClearCount + "\n");
                        fboObj.drawcall = 0; fboObj.programSwitch = 0; fboObj.clearCount = 0; fboObj.invalidClearCount = 0;
                        continue;
                    }
                    if (bShowLog) {
                        for (var attachment in fboObj.attachments) {
                            var attachmentObj = fboObj.attachments[attachment];
                            if (attachmentObj.mode == 0x8D41 /*GL_RENDERBUFFER*/) {
                                var rb = webglStats.renderbufferMap[attachmentObj.value];
                                var internalFormat = rb.internalFormat;
                                if (rb.width > webglStats.RTSizeLimit && rb.height > webglStats.RTSizeLimit) {
                                    warningLog += getAttachmentName(attachment) + " RT 尺寸超过 " + webglStats.RTSizeLimit + ", 请尽量降低渲染分辨率.\n";
                                }
                                logStr += ("--rb: " + getAttachmentName(attachment) + " [" + rb.width + "," + rb.height + "]," + getInternalFormatName(internalFormat) + ",dc:" + fboObj.drawcall + ", program: " + fboObj.programSwitch + ",clear:" + fboObj.clearCount + ",invalidClear:"+fboObj.invalidClearCount + ",sample:" + rb.sample + "," + "\n");
                            } 
                            else {
                                var tex = webglStats.texMap[attachmentObj.value];
                                if (tex) {
                                    var internalFormat = tex.internalFormat;
                                    if (tex.width > webglStats.RTSizeLimit && tex.height > webglStats.RTSizeLimit) {
                                        warningLog += getAttachmentName(attachment) + " RT 尺寸超过 " + webglStats.RTSizeLimit + ", 请尽量降低渲染分辨率.\n";
                                    }
                                    logStr += ("--tex: " + getAttachmentName(attachment) + " [" + tex.width + "," + tex.height + "]," + getInternalFormatName(internalFormat) + ",dc:" + fboObj.drawcall + ", program: " + fboObj.programSwitch + ",clear:" + fboObj.clearCount + ",invalidClear:" + fboObj.invalidClearCount + ", mode:" + tex.mode + ",mip:"+tex.miplevel + "\n");
                                }
                                else {
                                    logStr += ("--tex:" + getAttachmentName(attachment) + " is undefined: " + attachmentObj.value + "\n");
                                }
                            }
                        }
                    }
                ...
                if (bShowLog) {
                    if (totalDc > webglStats.DrawCallLimit ) {
                            warningLog += "DrawCall 数量:" + totalDc + "超过" + webglStats.DrawCallLimit + ", 请尽可能减少或合并DrawCall";
                    }
                    console.log(logStr);
            ...
            on_glBindFramebuffer : function(target, framebuffer) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                if ( (target != 0x8CA8 /*GL_READ_FRAMEBUFFER*/ && webglStats.curFbo.target != target) || webglStats.curFbo.fbo != framebuffer) {
                webglStats.curFbo.target = target;
                webglStats.curFbo.fbo = framebuffer; //GL.framebuffers[framebuffer];
                webglStats.fboList[webglStats.curFboSwitch] = framebuffer;
                webglStats.fboMap[framebuffer] = webglStats.fboMap[framebuffer] || {};
                webglStats.fboMap[framebuffer].attachments = webglStats.fboMap[framebuffer].attachments || {};
                webglStats.fboMap[framebuffer].drawcall = 0;
                webglStats.fboMap[framebuffer].programSwitch = 0;
                webglStats.fboMap[framebuffer].clearCount = 0;
                webglStats.fboMap[framebuffer].clearMask = 0;
                webglStats.fboMap[framebuffer].invalidClearCount = 0;
                webglStats.curFboSwitch++;
                }
            },
            on_glFramebufferRenderbuffer : function(target, attachment, renderbuffertarget, renderbuffer) {
                if (target != 0x8CA8 /*GL_READ_FRAMEBUFFER*/) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var fbo = webglStats.curFbo.fbo;
                webglStats.fboMap[fbo] = webglStats.fboMap[fbo] || {};
                webglStats.fboMap[fbo].attachments = webglStats.fboMap[fbo].attachments || {};
                webglStats.fboMap[fbo].attachments[attachment] = {mode:renderbuffertarget, value:renderbuffer};
                webglStats.fboMap[fbo].drawcall = 0;
                webglStats.fboMap[fbo].programSwitch = 0;
                webglStats.fboMap[fbo].clearCount = 0;
                webglStats.fboMap[fbo].clearMask = 0;
                webglStats.fboMap[fbo].invalidClearCount = 0;
                }
            },
            on_glFramebufferTexture2D : function(target, attachment, textarget, texture, level) {
                if (target != 0x8CA8 /*GL_READ_FRAMEBUFFER*/) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var fbo = webglStats.curFbo.fbo;
                webglStats.fboMap[fbo] = webglStats.fboMap[fbo] || {};
                webglStats.fboMap[fbo].attachments = webglStats.fboMap[fbo].attachments || {};
                webglStats.fboMap[fbo].attachments[attachment] = {mode:textarget, value:texture};
                webglStats.fboMap[fbo].drawcall = 0;
                webglStats.fboMap[fbo].programSwitch = 0;
                webglStats.fboMap[fbo].clearCount = 0;
                webglStats.fboMap[fbo].clearMask = 0;
            ...
            on_glFramebufferTextureLayer : function(target, attachment, texture, level, layer) {
                if (target != 0x8CA8 /*GL_READ_FRAMEBUFFER*/) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var fbo = webglStats.curFbo.fbo;
                webglStats.fboMap[fbo] = webglStats.fboMap[fbo] || {};
                webglStats.fboMap[fbo].attachments = webglStats.fboMap[fbo].attachments || {};
                webglStats.fboMap[fbo].attachments[attachment] = {mode:0x8C1A/*GL_TEXTURE_2D_ARRAY*/, value:texture};
                webglStats.fboMap[fbo].drawcall = 0;
                webglStats.fboMap[fbo].programSwitch = 0;
                webglStats.fboMap[fbo].clearCount = 0;
                webglStats.fboMap[fbo].clearMask = 0;
            ...
            on_glRenderbufferStorage : function(x0, x1, x2, x3) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var renderbuffer = {};
                renderbuffer.sample = 1;
                renderbuffer.internalFormat = x1;
                renderbuffer.width = x2;
                renderbuffer.height = x3;
                webglStats.renderbufferMap[webglStats.curRenderBuffer] = renderbuffer;
            },
            on_glRenderbufferStorageMultisample : function(x0, x1, x2, x3, x4) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var renderbuffer = {};
                renderbuffer.sample = x1;
                renderbuffer.internalFormat = x2;
                renderbuffer.width = x3;
                renderbuffer.height = x4;
                webglStats.renderbufferMap[webglStats.curRenderBuffer] = renderbuffer;
            },
            on_glBindRenderbuffer : function(target, renderbuffer) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                webglStats.curRenderBuffer = renderbuffer;
            },
            on_glBindTexture : function(target, texture) {
                if (0x0DE1 /*TEXTURE_2D*/ == target) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                webglStats.curTex2D = texture;
                }
            },
            on_glTexImage2D : function(target, level, internalFormat, width, height, border, format, type, pixels) {
                //console.assert(target == GLctx.TEXTURE_2D);
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var tex = webglStats.texMap[webglStats.curTex2D] || {};
                tex.mode = 1;
                tex.miplevel = level;
                tex.width = width;
                tex.height = height;
                tex.internalFormat = internalFormat;
                webglStats.texMap[webglStats.curTex2D] = tex;
            },
            on_glCompressedTexImage2D : function(target, level, internalformat, width, height, border, imageSize, data) {
                //console.assert(target == GLctx.TEXTURE_2D);
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var tex = webglStats.texMap[webglStats.curTex2D] || {};
                tex.mode = 2;
            ...
            on_glTexStorage2D : function(x0, x1, x2, x3, x4) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var tex = webglStats.texMap[webglStats.curTex2D] || {};
                tex.mode = 1;
                tex.miplevel = x1;
                tex.internalFormat = x2;
                tex.width = x3;
                tex.height = x4;
                webglStats.texMap[webglStats.curTex2D] = tex;
            },
            on_glDrawArrays : function(mode, first, count) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var fbo = webglStats.curFbo.fbo;
                var fboObj = webglStats.fboMap[fbo];
                fboObj.drawcall += 1;
            },
            on_glDrawArraysInstanced : function(mode, first, count, primcount) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var fbo = webglStats.curFbo.fbo;
                var fboObj = webglStats.fboMap[fbo];
                fboObj.drawcall += 1;
            },
            on_glDrawElements : function(mode, count, type, indices) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var fbo = webglStats.curFbo.fbo;
                var fboObj = webglStats.fboMap[fbo];
                fboObj.drawcall += 1;
            },
            on_glDrawElementsInstanced : function(mode, count, type, indices, primcount) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var fbo = webglStats.curFbo.fbo;
                var fboObj = webglStats.fboMap[fbo];
                fboObj.drawcall += 1;
            },
            on_glClear : function(x0) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                var fbo = webglStats.curFbo.fbo;
                var fboObj = webglStats.fboMap[fbo];
                if (fboObj){
                    fboObj.clearCount += 1;
                    fboObj.clearMask = fboObj.clearMask | x0;
                    if (fboObj.drawcall > 0) {
                        fboObj.invalidClearCount += 1;
                    }
                }
            },
            on_glUseProgram : function(program) {
                var webglStats = GameGlobal.unityNamespace.ProfileWebgl;
                if (webglStats.curProgram != program) {
                    webglStats.curProgramSwitch += 1;
                    webglStats.curProgram = program;
                    var fbo = webglStats.curFbo.fbo;
                    var fboObj = webglStats.fboMap[fbo];
                    fboObj.programSwitch += 1;
                }
            },
            enableProfile : false,
            fboMap : {},
            ...
            curRenderBuffer: 0,
            curTex2D: 0,
            curFbo: { target:null, fbo: 0 },
            fboList: [],
            curProgram: 0,
            curFboSwitch: 0,
            curProgramSwitch: 0,
            curFboDrawCall: 0,
            showLogInterval: 3600,
            lastShowLogFrame: 0
...
var abort = function(what) {
 ...
 ABORT = true;
 EXITSTATUS = 1;
 if (typeof ENVIRONMENT_IS_PTHREAD !== "undefined" && ENVIRONMENT_IS_PTHREAD) err("Pthread aborting at " + new Error().stack);
 if (what !== undefined) {
  out(what);
  err(what);
...
if (ENVIRONMENT_IS_NODE) {
 ...
 if (ENVIRONMENT_IS_WORKER) {
  scriptDirectory=this.location.href;
...
function Pointer_stringify(ptr){return UTF8ToString(ptr)}function UTF8ToString(ptr, maxBytesToRead) {
...
function initRuntime() {
 ...
 if(typeof _WXFSInit!=="undefined")_WXFSInit(unityNamespace.ttlAssetBundle?unityNamespace.ttlAssetBundle:5);TTY.init();
...
var runDependencies = GameGlobal.unityNamespace.runDependencies = 0;
...
function addRunDependency(id) {GameGlobal.manager.Logger.eventLog("addRunDependency: ", id);
 ...
 if (Module["monitorRunDependencies"]) {
  Module["monitorRunDependencies"](runDependencies, id);
 }
 if (id) {
...
function getBinary(file) {return;
 try {
...
function createWasm() {
 ...
 function receiveInstance(instance, module) {
  ...
  if(Module.wasmInstantiated){Module.wasmInstantiated();removeRunDependency("wasm-instantiate")};
 }
 addRunDependency("wasm-instantiate");addRunDependency("wasm-preloadAssets");GameGlobal.manager.TimeLogger.timeStart("wasm编译耗时");;
 ...
 function instantiateArrayBuffer(receiver) {
  return getBinaryPromise().then(function(binary) {
   if(Module["wasmBin"]){return WebAssembly.instantiate(Module["wasmBin"], info);}return WebAssembly.instantiate(Module["wasmPath"], info);
   return result;
...
function _JS_CallAsLongAsNoExceptionsSeen(cb) {
 if (!ExceptionsSeen) {
  try {
   ...
   ExceptionsSeen = 1;
   err("Uncaught exception from main loop:");
   err(e);
   err("Halting program.");
...
function jsCanvasSelector() {
 if (Module["canvas"] && !Module.IsWxGame && !Module["canvas"].id) throw 'Module["canvas"] must have a CSS ID associated with it!';
...
function _JS_FileSystem_Initialize(){if (!Module.indexedDB) return;if (!Module.indexedDB) return;
 Module.setInterval(function() {
  fs.sync(true);
 }, fs.syncInternal);
}

function _JS_FileSystem_Sync(){if (!Module.indexedDB) return;
 if(Module.IsWxGame)return;fs.sync(false);
}
...
function _JS_Init_ContextMenuHandler() {if(Module.IsWxGame)return;
...
function _JS_Log_Dump(ptr, type) {
 ...
 default:
  err("Unknown console message type!");
  err(str);
 }
}
...
function _JS_SystemInfo_GetDocumentURL(buffer, bufferSize) {
 if (buffer) stringToUTF8(GameGlobal.unityNamespace.DATA_CDN || "https://game.weixin.qq.com", buffer, bufferSize);
 return lengthBytesUTF8(GameGlobal.unityNamespace.DATA_CDN || "https://game.weixin.qq.com");
}
...
function _JS_SystemInfo_GetStreamingAssetsURL(buffer,bufferSize){if(Module.IsWxGame) Module.streamingAssetsUrl=Module.resolveBuildUrl("StreamingAssets");if(Module.IsWxGame) Module.streamingAssetsUrl=Module.resolveBuildUrl("StreamingAssets");
...
function _JS_SystemInfo_HasAstcHdr() {
 ...
 if (ext && ext.getSupportedProfiles) {
  return Module.IsWxGame ? false:ext.getSupportedProfiles().includes("hdr");
 }
 return false;
}
...
function _JS_WebRequest_Create(url, method) {
 ...
 var abortController = new GameGlobal.unityNamespace.UnityLoader.UnityCache.XMLHttpRequest();if(GameGlobal.TEXTURE_PARALLEL_BUNDLE){GameGlobal.ParalleLDownloadTexture(_url)}
 var requestOptions = {
  url: _url,
  init: {
   method: _method,
...
function _reTryRequest(requestId,arg,onresponse,needRetry){var abortController = wr.abortControllers[requestId];abortController.retryCount = abortController.retryCount || 0;abortController.retryCount++;var xml=new GameGlobal.unityNamespace.UnityLoader.UnityCache.XMLHttpRequest();xml.open('GET',abortController.paramsCache.url,true);xml.responseType=abortController.responseType;xml.onload=function(){if(abortController.status>=400&&needRetry){setTimeout(function(){_reTryRequest(requestId,arg,onresponse)},1000);return false;}else{if(onresponse){var kWebRequestOK=0;var byteArray=new Uint8Array(xml.response);if(byteArray.length!=0){var buffer=_malloc(byteArray.length);HEAPU8.set(byteArray,buffer);dynCall('viiiiii',onresponse,[arg,xml.status,buffer,byteArray.length,0,kWebRequestOK])}else{dynCall('viiiiii',onresponse,[arg,xml.status,0,0,0,kWebRequestOK])}}}};xml.send(abortController.postData);xml.onerror=abortController.onerror;xml.ontimeout=abortController.ontimeout;xml.onabort=abortController.onabort;console.error('load url error:' + abortController.paramsCache.url);GameGlobal.logmanager.warn('load url error:'+abortController.paramsCache.url);GameGlobal.realtimeLogManager.error('load url error:'+abortController.paramsCache.url);} function _JS_WebRequest_Send(requestId, ptr, length, arg, onresponse, onprogress) {
 ...
 function HandleSuccess(response, body) {
  ...
  HandleProgress({response,loaded: true,lengthComputable: true,total: 100,type: 'progress'});var kWebRequestOK = 0;if (requestOptions.init.enableStreamingDownload) {
   dynCall("viiiiii", onresponse, [ arg, response.status, 0, body.length, 0, kWebRequestOK ]);
  } else if(typeof WXFS!=="undefined"&&WXFS.isWXAssetBundle(response.url)){var http=response.originXHR;if(body.length==0||http.status>=400){GameGlobal.manager.reporter.wxAssetBundle.reportEmptyContent({stage:WXFS.WXABErrorSteps['kWebRequestResponse'],fromCache:http.isReadFromCache,httpStatus:http.status,path:http.paramsCache.url,size:0});GameGlobal.manager.Logger.pluginLog('[WXAssetBundle]WebRequest status: '+http.status+', url: '+http.paramsCache.url+' return size '+body.length+' from '+(http.isReadFromCache?'Cache':'CDN'));dynCall("viiiiii", onresponse,[arg,response.status,_malloc(1),1,0,kWebRequestOK]);return}http.onsave=function http_onsave(e){WXFS.cache.cleanable(WXFS.path2fd.get(e));};var arrayBuffer=http.response;let path=WXFS.url2path(response.url);let numberfd=WXFS.path2fd.get(path);if(numberfd==undefined){numberfd=WXFS.newfd();WXFS.path2fd.set(path,numberfd);}let wxStream=WXFS.fd2wxStream.get(numberfd);wxStream={node:{mode:32768,usedBytes:body.length},fd:numberfd,path:path,seekable:true,position:0,stream_ops:MEMFS.stream_ops,ungotten:[],error:false};wxStream.stream_ops.read=WXFS.read;WXFS.fd2wxStream.set(numberfd,wxStream);WXFS.cache.put(numberfd,arrayBuffer,http.isReadFromCache);dynCall("viiiiii",onresponse,[arg,http.status,0,0,0,kWebRequestOK]);WXFS.disk.set(unityNamespace.PathInFileOS(path),body.length);}else if(!abortController.ignoreCallback&&body.length!=0){
 ...
 try {
  if (length > 0) {
   ...
   requestOptions.init.body = postData;
  }
  ...
  fetchImpl = abortController.openAndSend;fetchImpl(requestOptions.url, requestOptions.init).then(function(response) {
   wr.responses[requestId] = response;
...
function _JS_WebRequest_SetTimeout(requestId, timeout) {
 ...
 requestOptions.init.timeout = timeout;requestOptions.timeout = timeout;
}
...
function _WXGetTotalMemorySize() {
 ...
 err("Fail to find wasmMemory.buffer, TotalMemorySize is not correct.");
 return 0;
}
...
function _WXProfilingMemoryDump() {
 ...
 err("Please call WX.InitSDK & Select ProfilingMemory Option");
}
...
function getRandomDevice(){ if(Module.IsWxGame)return function () {return Math.random() * 256 | 0};
...
var MEMFS = {
 ...
 stream_ops: {
  ...
  write: function(stream, buffer, offset, length, position, canOwn) {
   ...
   if (!Module.IsWxGame && buffer.buffer === HEAP8.buffer) {
    canOwn = false;
   }
   if (!length) return 0;
...
var IDBFS = GameGlobal.unityNamespace.IDBFS = {
 dbs: {},
 indexedDB: function() {
...
var FS = GameGlobal.unityNamespace.FS= {
 root: null,
 mounts: [],
 devices: {},
 streams: [],
...
var SYSCALLS = {
 ...
 getStr: function(ptr) {
  var ret=UTF8ToString(ptr);if(typeof WXFS!=="undefined"&&WXFS.isWXAssetBundle(ret)){return WXFS.url2path(ret);}
  return ret;
 },
 getStreamFromFD:function(fd){if(fd>FS.MAX_OPEN_FDS){var stream=WXFS.fd2wxStream.get(fd);if(stream==undefined)throw new FS.ErrnoError(8);return stream;}
  var stream = FS.getStream(fd);
  if (!stream) throw new FS.ErrnoError(8);
  return stream;
 },
...
function ___sys_access(path, amode) {
 try {
  ...
  if(typeof WXFS!=="undefined"&&WXFS.isWXAssetBundle(path)){if(WXFS.path2fd.has(path))return 0;return WXFS.doWXAccess(path,amode)}return SYSCALLS.doAccess(path,amode);
 } catch (e) {
  if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e);
  return -e.errno;
 }
...
function ___sys_fstat64(fd, buf) {
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if(fd>FS.MAX_OPEN_FDS){return SYSCALLS.doStat(WXFS.wxstat,stream.path,buf);}return SYSCALLS.doStat(FS.stat,stream.path,buf);
 } catch (e) {
  if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e);
  return -e.errno;
 }
...
function ___sys_open(path, flags, varargs) {
 SYSCALLS.varargs = varargs;
 try {
  var pathname=SYSCALLS.getStr(path);if(typeof WXFS!=="undefined"&&WXFS.isWXAssetBundle(pathname)){var numberfd=WXFS.path2fd.get(pathname);if(numberfd!==undefined){return numberfd;}const res=WXFS.LoadBundleFromFile(pathname);numberfd=WXFS.newfd();let wxStream={fd:numberfd,path:pathname,flags:flags,seekable:true,position:0,stream_ops:MEMFS.stream_ops,ungotten:[],node:{mode:32768,usedBytes:new Uint8Array(res).length},error:false,};wxStream.stream_ops.read=WXFS.read;WXFS.path2fd.set(pathname,numberfd);WXFS.fd2wxStream.set(numberfd,wxStream);WXFS.cache.put(numberfd,res);return numberfd}
...
function ___sys_stat64(path, buf) {
 try {
  path = SYSCALLS.getStr(path);
  if(typeof WXFS!=="undefined"&&WXFS.isWXAssetBundle(path)){return SYSCALLS.doStat(WXFS.wxstat,path,buf);}return SYSCALLS.doStat(FS.stat,path,buf);
 } catch (e) {
  if (typeof FS === "undefined" || !(e instanceof FS.ErrnoError)) abort(e);
  return -e.errno;
 }
...
function _emscripten_set_main_loop_timing(mode, value) {
 ...
 if (mode == 0) {
  ...
  Browser.mainLoop.timingValue=1;_WX_SetPreferredFramesPerSecond(60/value);Browser.mainLoop.scheduler=function Browser_mainLoop_scheduler_rAF() {
...
function setMainLoop(browserIterationFunc, fps, simulateInfiniteLoop, arg, noSetTiming) {
 ...
 Browser.mainLoop.runner = function Browser_mainLoop_runner() {
  ...
  GameGlobal.unityNamespace.ProfileWebgl.beginFrame();GL.newRenderingFrameStarted();
  if (Browser.mainLoop.method === "timeout" && Module.ctx) {
   warnOnce("Looks like you are rendering without using requestAnimationFrame for the main loop. You should use 0 for the frame rate in emscripten_set_main_loop in order to use requestAnimationFrame, as that can greatly improve your frame rates!");
   Browser.mainLoop.method = "";
  }
  if(GameGlobal.manager.isVisible){           
                        if(typeof GameGlobal.calcFrameTimeFunc != 'undefined')
                        {
                            let frameStart = performance.now();
                            Browser.mainLoop.runIter(browserIterationFunc);
GameGlobal.unityNamespace.ProfileWebgl.endFrame();
                            GameGlobal.calcFrameTimeFunc(frameStart, performance.now());
                        }
                        else {
                            Browser.mainLoop.runIter(browserIterationFunc);
GameGlobal.unityNamespace.ProfileWebgl.endFrame();
 ...
 if (!noSetTiming) {
  if (fps && fps > 0) _emscripten_set_main_loop_timing(0, 1e3 / fps); else _emscripten_set_main_loop_timing(1, 1);if (!GameGlobal.unityNamespace.isLoopRunnerEnable) return;;
  Browser.mainLoop.scheduler();
 }
...
var Browser = GameGlobal.unityNamespace.Browser = {
 mainLoop: {
  running: false,
  scheduler: null,
  method: "",
 ...
 createContext: function(canvas, useWebGL, setInModule, webGLContextAttributes) {
  ...
  if (useWebGL) {
   ...
   if (typeof GL !== "undefined") {
    contextHandle = GL.createContext(canvas, contextAttributes);WXWASMSDK.canvasContext && WXWASMSDK.canvasContext._triggerCallback();
    if (contextHandle) {
 ...
 updateCanvasDimensions: function(canvas, wNative, hNative) {
  ...
  if (Browser.resizeCanvas) {
   ...
   if (typeof canvas.style != "undefined") {
    if(canvas.style.removeProperty)canvas.style.removeProperty("width");
    if(canvas.style.removeProperty)canvas.style.removeProperty("height");
   }
  } else {
   ...
   if (typeof canvas.style != "undefined") {
    if (w != wNative || h != hNative) {
     if(canvas.style.setProperty)canvas.style.setProperty("width", w + "px", "important");
     if(canvas.style.setProperty)canvas.style.setProperty("height", h + "px", "important");
    } else {
     if(canvas.style.removeProperty)canvas.style.removeProperty("width");
     if(canvas.style.removeProperty)canvas.style.removeProperty("height");
    }
...
var JSEvents = {
 ...
 registerOrRemoveHandler: function(eventHandler) {
  ...
  if (eventHandler.callbackfunc) {
   eventHandler.eventListenerFunc=jsEventHandler;if(eventHandler.eventTypeString==="blur"){wx.onHide(jsEventHandler);}else if(eventHandler.eventTypeString==="focus"){wx.onShow(jsEventHandler);}else 
...
function findEventTarget(target) {return Module['canvas'];
 target = maybeCStringToJsString(target);
 var domElement = specialHTMLTargets[target] || (typeof document !== "undefined" ? document.querySelector(target) : undefined);
 return domElement;
}

function findCanvasEventTarget(target) {return Module['canvas'];
...
function _emscripten_get_num_gamepads(){ if(Module.IsWxGame)return 0;
 if (!JSEvents.lastGamepadState) throw "emscripten_get_num_gamepads() can only be called after having first called emscripten_sample_gamepad_data() and that function has returned EMSCRIPTEN_RESULT_SUCCESS!";
 return Module.IsWxGame ? 0 : JSEvents.lastGamepadState.length;
}
...
function registerFocusEventCallback(target, userData, useCapture, callbackfunc, eventTypeId, eventTypeString, targetThread) {
 ...
 var focusEventHandlerFunc=function(ev){var focusEvent=JSEvents.focusEvent;dynCall_iiii(callbackfunc,eventTypeId,focusEvent,userData);return;
  var e = ev || event;
...
var GL = {
 ...
 createContext: function(canvas, webGLContextAttributes) {
  if (!canvas.getContextSafariWebGL2Fixed) {
   ...
   canvas.getContext = function(ver, attrs) {
    ...
    if(Module.IsWxGame)return gl;return ver == "webgl" == gl instanceof WebGLRenderingContext ? gl : null;
   };
  }
 ...
 initExtensions: function(context) {
  ...
  var exts = GLctx.getSupportedExtensions() || [];if(GameGlobal.USED_TEXTURE_COMPRESSION) { exts.push('WEBGL_compressed_texture_etc1'); exts.push('WEBGL_compressed_texture_etc');}
...
function _emscripten_webgl_do_create_context(target, attributes) {
 ...
 var contextHandle = GL.createContext(canvas, contextAttributes);WXWASMSDK.canvasContext && WXWASMSDK.canvasContext._triggerCallback();
...
function _fd_close(fd){if(fd>FS.MAX_OPEN_FDS){return 0}
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
GameGlobal.unityNamespace.ProfileWebgl.on_glBindFramebuffer(target, framebuffer);
 GLctx.bindFramebuffer(target, GL.framebuffers[framebuffer]);
}

function _glBindRenderbuffer(target, renderbuffer) {
GameGlobal.unityNamespace.ProfileWebgl.on_glBindRenderbuffer(target, renderbuffer);
...
GameGlobal.unityNamespace.ProfileWebgl.on_glBindTexture(target, texture);
...
GameGlobal.unityNamespace.ProfileWebgl.on_glClear(x0);
 GLctx["clear"](x0);
}
...
function _glClientWaitSync(sync, flags, timeoutLo, timeoutHi) {
 return 1?0x911A:GLctx.clientWaitSync(GL.syncs[sync], flags, convertI32PairToI53(timeoutLo, timeoutHi));
}
...
GameGlobal.unityNamespace.ProfileWebgl.on_glCompressedTexImage2D(target, level, internalFormat, width, height, border, imageSize, data);
...
GameGlobal.unityNamespace.ProfileWebgl.on_glDrawArrays(mode, first, count);
...
GameGlobal.unityNamespace.ProfileWebgl.on_glDrawArraysInstanced(mode, first, count, primcount);
...
GameGlobal.unityNamespace.ProfileWebgl.on_glDrawElements(mode, count, type, indices);
 var buf;
...
GameGlobal.unityNamespace.ProfileWebgl.on_glDrawElementsInstanced(mode, count, type, indices, primcount);
...
GameGlobal.unityNamespace.ProfileWebgl.on_glFramebufferRenderbuffer(target, attachment, renderbuffertarget, renderbuffer);
 GLctx.framebufferRenderbuffer(target, attachment, renderbuffertarget, GL.renderbuffers[renderbuffer]);
}

function _glFramebufferTexture2D(target, attachment, textarget, texture, level) {
GameGlobal.unityNamespace.ProfileWebgl.on_glFramebufferTexture2D(target, attachment, textarget, texture, level);
 GLctx.framebufferTexture2D(target, attachment, textarget, GL.textures[texture], level);
}

function _glFramebufferTextureLayer(target, attachment, texture, level, layer) {
GameGlobal.unityNamespace.ProfileWebgl.on_glFramebufferTextureLayer(target, attachment, texture, level, layer);
...
function _glReadPixels(x, y, width, height, format, type, pixels) {
 if (GL.currentContext.version >= 2) {
  if (GLctx.currentPixelPackBufferBinding) {
   ...
   var heap = heapObjectForWebGLType(type);
   GLctx.readPixels(x, y, width, height, format, type, emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, 0));
  }
  return;
 }
...
GameGlobal.unityNamespace.ProfileWebgl.on_glRenderbufferStorage(x0, x1, x2, x3);
 GLctx["renderbufferStorage"](x0, x1, x2, x3);
}

function _glRenderbufferStorageMultisample(x0, x1, x2, x3, x4) {
GameGlobal.unityNamespace.ProfileWebgl.on_glRenderbufferStorageMultisample(x0, x1, x2, x3, x4);
...
GameGlobal.unityNamespace.ProfileWebgl.on_glTexImage2D(target, level, internalFormat, width, height, border, format, type, pixels);
 if (GL.currentContext.version >= 2) {
  if (GLctx.currentPixelUnpackBufferBinding) {
   GLctx.texImage2D(target, level, internalFormat, width, height, border, format, type, pixels);
  } else if (pixels) {
   var heap = heapObjectForWebGLType(type);
   GLctx.texImage2D(target, level, internalFormat, width, height, border, format, type, emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, 0));
  } else {
...
function _glTexImage3D(target, level, internalFormat, width, height, depth, border, format, type, pixels) {
 if (GLctx.currentPixelUnpackBufferBinding) {
  ...
  var heap = heapObjectForWebGLType(type);
  GLctx["texImage3D"](target, level, internalFormat, width, height, depth, border, format, type, emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, 0));
 } else {
...
GameGlobal.unityNamespace.ProfileWebgl.on_glTexStorage2D(x0, x1, x2, x3, x4);window._lastTexStorage2DParams = [x0, x1, x2, x3, x4];if(x2 == 36196 || x2 == 37492 || x2 == 37493){return;}
...
function _glTexSubImage3D(target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, pixels) {
 if (GLctx.currentPixelUnpackBufferBinding) {
  ...
  var heap = heapObjectForWebGLType(type);
  GLctx["texSubImage3D"](target, level, xoffset, yoffset, zoffset, width, height, depth, format, type, emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, 0));
 } else {
...
GameGlobal.unityNamespace.ProfileWebgl.on_glUseProgram(program);
 program = GL.programs[program];
...
var asmLibraryArg = {
 ...
 "JS_Sound_ResumeIfNeeded":window.WXWASMSDK._JS_Sound_ResumeIfNeeded,
...
if (!Object.getOwnPropertyDescriptor(Module, "intArrayToString")) Module["intArrayToString"] = function() {
 ...
 Module['GL'] = GL; Module['ccall'] = ccall;
...
function callMain(args) {
 ...
 try {
  ...
  if(ABORT===true)return;calledMain = true;if(Module.calledMainCb){Module.calledMainCb()};if (GameGlobal.unityNamespace.enableProfileStats) {setTimeout(() => {SendMessage('WXSDKManagerHandler', 'OpenProfileStats');}, 10000);};
 }
}
...
function run(args) {
 ...
 function doRun() {GameGlobal.manager.TimeLogger.timeStart("callMain耗时");
  if (calledRun) return;
  calledRun = true;
